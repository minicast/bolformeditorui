...
var KeyboardPanel = React.createClass({
  ...
  render: function render() {
    return React.createElement(
      ...
      React.createElement(
        ...
        React.createElement("div", { id: "keyboard-editor", style: { height: 190 + "px" } })
      )
    );
  },
    ...
    keyboardEditor.getSession().on('change', function (e) {
      ...
      if (keyParsed) {
        var update = function update(source) {
          ...
          nodeEnter.append("rect").attr("width", function (d) {
            if (this.previousElementSibling.clientWidth) {
              return this.previousElementSibling.clientWidth + 10;
            } else {
              //firefox (gecko-based browsers) & edge (all ie-brosers) have clientWidth 0
          ...
          }).attr('rx', 10).attr('ry', 10).style("fill", function (d) {
            if (d.children) {
              return 'DarkSeaGreen';
            } else {
              return 'CadetBlue';
            }
          }).style('opacity', '0.5').attr('transform', function (d) {
            var xdev, ydev;
            if (this.previousElementSibling.clientWidth) {
        ...
        // .append("g")
        .insert("g").attr("width", width).attr("height", height).attr("transform", "translate(" + (margin.left + margin.left) + "," + (margin.top + margin.bottom + 10) + ")");
